...
export function QuickPredictionWizard({
  ...
  return (
    <>
      <button
        className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-md bg-gold px-5 text-base font-black text-pitch shadow-[0_16px_50px_rgba(244,201,93,0.24)] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-45 sm:w-auto"
        type="button"
        onClick={() => {
          setIndex(0);
...
function QuickPredictionStep({
  ...
  const isGroupStage = predictionType === "group_stage";
  ...
  async function saveCurrentPrediction() {
    ...
    formData.set("predictedWinnerTeamId", winnerTeamId);
    formData.set("predictedHomeScore", homeScore);
    formData.set("predictedAwayScore", awayScore);
    formData.set("predictsExtraTime", String(extraTime));
    formData.set("predictsPenalties", String(penalties));

    const result = await saveQuickPrediction(formData);
    setIsSaving(false);
  ...
  return (
    <>
      ...
      {isGroupStage ? (
        ...
        <div className="grid gap-2">
          <p className="text-sm font-black text-white">¿Quién avanza?</p>
      ...
      {!isGroupStage ? (
        <div className="grid grid-cols-2 gap-2">
          <Toggle label="Prórroga" checked={extraTime} onChange={setExtraTime} />
          <Toggle label="Penales" checked={penalties} onChange={setPenalties} />
        </div>
      ) : null}
